      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-[1.3fr_1fr]">
        <div className="min-w-0">
          ...
          <ul className="mt-5 space-y-2 text-sm">
            ...
            <li className="flex items-center gap-2">
              ...
              <a className="hover:underline" href={`tel:${STORE.phoneTel}`}>
                {STORE.phone}
              </a>
            </li>
            <li className="flex items-center gap-2 min-w-0">
              <Mail className="h-4 w-4 shrink-0 text-accent" />
              <a className="truncate hover:underline" href={`mailto:${STORE.email}`}>
                {STORE.email}
              </a>
            </li>
          </ul>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <li>
              <Link to="/proizvodi" search={(p) => p} className="hover:text-primary">
                {tr(t.nav.products)}
              </Link>
            </li>
            <li>
              <Link to="/" hash="bio-match" search={(p) => p} className="hover:text-primary">
                {tr(t.nav.quiz)}
              </Link>
            </li>
            <li>
              <Link to="/prica" search={(p) => p} className="hover:text-primary">
                {tr(t.nav.story)}
              </Link>
            </li>
            <li>
              <Link to="/kontakt" search={(p) => p} className="hover:text-primary">
                {tr(t.nav.contact)}
              </Link>
            </li>
          </ul>
